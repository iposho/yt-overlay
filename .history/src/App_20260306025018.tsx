    ...
    const fetchWeather = async () => {
      try {
        ...
        if (geoData.results && geoData.results.length > 0) {
          ...
          if (weatherJson.current || weatherJson.current_weather) {
            const current = weatherJson.current;
            const curWeather = weatherJson.current_weather;

            setWeatherData({
              temp: Math.round(current?.temperature_2m ?? curWeather?.temperature ?? 0),
              code: current?.weather_code ?? curWeather?.weathercode ?? 0,
              humidity: current?.relative_humidity_2m,
              windSpeed: current?.wind_speed_10m ?? curWeather?.windspeed ?? 0,
              windDegree: current?.wind_direction_10m ?? curWeather?.winddirection ?? 0,
            });
          }
        }
      } catch (e) {
