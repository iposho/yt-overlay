      ...
      try {
        ...
        if (geoData.results && geoData.results.length > 0) {
          ...
          const weatherRes = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m,wind_direction_10m&current_weather=true`,
          );
          const weatherJson = await weatherRes.json();
          setWeatherData({
            temp: Math.round(weatherJson.current.temperature_2m ?? weatherJson.current_weather.temperature),
            code: weatherJson.current.weather_code ?? weatherJson.current_weather.weathercode,
            humidity: weatherJson.current.relative_humidity_2m,
            windSpeed: weatherJson.current.wind_speed_10m ?? weatherJson.current_weather.windspeed,
            windDegree: weatherJson.current.wind_direction_10m ?? weatherJson.current_weather.winddirection,
          });
        }
      } catch (e) {
      ...
      <AnimatePresence>
        {state.widgets.weather.enabled && weatherData && (
          <motion.div
            initial={{ y: -50, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            className="overlay-widget weather-widget enhanced"
            style={{ color: state.theme.text }}
          >
            ...
            <div className="weather-details">
              {weatherData.humidity !== undefined && (
                <div className="detail-item">
                  <Droplets size={12} style={{ color: state.theme.primary }} />
                  <span>{weatherData.humidity}%</span>
                </div>
              )}
              {weatherData.windSpeed !== undefined && (
                <div className="detail-item">
                  <Wind size={12} style={{ color: state.theme.primary }} />
                  <span>{Math.round(weatherData.windSpeed)}<small>km/h</small></span>
                </div>
              )}
